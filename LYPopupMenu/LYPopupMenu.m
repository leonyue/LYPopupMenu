...
static const NSTimeInterval kLYPopupMenuAnimationDuration = 0.2;

@interface LYPopupMenu ()

@property (nonatomic, assign, getter = isVisible, readwrite) BOOL visible;
@property (nonatomic, strong) LYPopupMenuOverlayView *overlayView;

@property (nonatomic, weak) UIView *view;
@property (nonatomic, assign) CGRect targetRect;

@property (nonatomic, assign) NSUInteger page;
@property (nonatomic, assign) LYPopupMenuArrowDirection actualArrorDirection;
...
+ (Class)itemViewClass
{
    return [LYPopupMenuItemView class];
}

+ (Class)pagenatorViewClass
{
    return [LYPopupMenuPagenatorView class];
}
...
- (instancetype)initWithItems:(NSArray *)items
{
    ...
    if (self) {
        ...
        self.cornerRadius = 8;
        self.arrowSize = 9;
        self.arrowDirection = LYPopupMenuArrowDirectionDefault;
...
- (void)showInView:(UIView *)view targetRect:(CGRect)targetRect animated:(BOOL)animated
{
    ...
    LYPopupMenuArrowDirection arrowDirection = self.arrowDirection;
    
    if (arrowDirection == LYPopupMenuArrowDirectionDefault) {
        if ((targetRect.origin.y - (self.height + self.arrowSize)) >= self.popupMenuInsets.top) {
            arrowDirection = LYPopupMenuArrowDirectionDown;
        }
            ...
            arrowDirection = LYPopupMenuArrowDirectionUp;
        }
        else {
            ...
            arrowDirection = (left > right) ? LYPopupMenuArrowDirectionLeft : LYPopupMenuArrowDirectionRight;
        }
    }
    ...
    switch (arrowDirection) {
        case LYPopupMenuArrowDirectionDown:
        case LYPopupMenuArrowDirectionUp:
        ...
        case LYPopupMenuArrowDirectionLeft:
            maximumWidth = targetRect.origin.x - self.popupMenuInsets.left;
            break;
            
        case LYPopupMenuArrowDirectionRight:
    ...
    // Create overlay view
    self.overlayView = ({
        LYPopupMenuOverlayView *overlayView = [[LYPopupMenuOverlayView alloc] initWithFrame:view.bounds];
    ...
    if (animated) {
        ...
        [UIView animateWithDuration:kLYPopupMenuAnimationDuration animations:^(void) {
            self.alpha = 1.0;
...
- (void)dismissAnimated:(BOOL)animated
{
    ...
    if (animated) {
        [UIView animateWithDuration:kLYPopupMenuAnimationDuration animations:^{
            self.alpha = 0;
        } completion:^(BOOL finished) {
...
- (void)createItemViews
{
    ...
    for (LYPopupMenuItem *item in self.items) {
        LYPopupMenuItemView *itemView = [[[self class] itemViewClass] itemViewWithItem:item];
...
- (void)resetItemViewState:(LYPopupMenuItemView *)itemView
{
...
- (void)groupItemViewsWithMaximumWidth:(CGFloat)maximumWidth
{
    ...
    CGFloat pagenatorWidth = [LYPopupMenuPagenatorView pagenatorWidth];
    
    // Create new array
    NSMutableArray *itemViews = [NSMutableArray array];
    CGFloat width = 0;
    if (self.actualArrorDirection == LYPopupMenuArrowDirectionLeft || self.actualArrorDirection == LYPopupMenuArrowDirectionRight) {
        width += self.arrowSize;
    }
    
    for (LYPopupMenuItemView *itemView in self.itemViews) {
        // Clear state
...
- (void)updateVisibleItemViewsWithPage:(NSUInteger)page
{
    ...
    if (numberOfPages > 1 && page != 0) {
        LYPopupMenuPagenatorView *leftPagenatorView = [[[self class] pagenatorViewClass] leftPagenatorViewWithTarget:self action:@selector(showPreviousPage)];
    ...
    for (LYPopupMenuItemView *itemView in itemViews) {
        [self addSubview:itemView];
        [visibleItemViews addObject:itemView];
    }
    
    if (numberOfPages > 1 && page != numberOfPages - 1) {
        LYPopupMenuPagenatorView *rightPagenatorView = [[[self class] pagenatorViewClass] rightPagenatorViewWithTarget:self action:@selector(showNextPage)];
...
- (void)layoutVisibleItemViews
{
    ...
    if (self.actualArrorDirection == LYPopupMenuArrowDirectionDown || self.actualArrorDirection == LYPopupMenuArrowDirectionUp) {
        height += self.arrowSize;
    }
    
    CGFloat offset = 0;
    for (NSInteger i = 0; i < self.visibleItemViews.count; i++) {
        LYPopupMenuItemView *itemView = [self.visibleItemViews objectAtIndex:i];
        
        // Clear state
        [self resetItemViewState:itemView];
        
        // Set item view insets
        if (i == 0 && self.actualArrorDirection == LYPopupMenuArrowDirectionLeft) {
            itemView.button.contentEdgeInsets = UIEdgeInsetsMake(0, self.arrowSize, 0, 0);
        }
        else if (i == self.visibleItemViews.count - 1 && self.actualArrorDirection == LYPopupMenuArrowDirectionRight) {
            itemView.button.contentEdgeInsets = UIEdgeInsetsMake(0, 0, 0, self.arrowSize);
        }
        else if (self.actualArrorDirection == LYPopupMenuArrowDirectionDown) {
            itemView.button.contentEdgeInsets = UIEdgeInsetsMake(0, 0, self.arrowSize, 0);
        }
        else if (self.actualArrorDirection == LYPopupMenuArrowDirectionUp) {
        ...
        if ((i == 0 && self.actualArrorDirection == LYPopupMenuArrowDirectionLeft) ||
            (i == self.visibleItemViews.count - 1 && self.actualArrorDirection == LYPopupMenuArrowDirectionRight)) {
            width += self.arrowSize;
        }
...
- (UIImage *)popupMenuImageWithHighlighted:(BOOL)highlighted
{
    ...
    // Draw body
    CGFloat y = (self.actualArrorDirection == LYPopupMenuArrowDirectionUp) ? self.arrowSize : 0;
    CGFloat height = self.height;
    ...
    for (NSInteger i = 0; i < self.visibleItemViews.count; i++) {
        ...
        if (i == 0) {
            if (self.visibleItemViews.count == 1) {
                ...
                CGRect bodyRect;
                CGRect tailRect;
                if (self.actualArrorDirection == LYPopupMenuArrowDirectionLeft) {
                ...
                else if (self.actualArrorDirection == LYPopupMenuArrowDirectionRight) {
            ...
            CGRect bodyRect;
            CGRect tailRect;
            if (self.actualArrorDirection == LYPopupMenuArrowDirectionRight) {
...
- (CGMutablePathRef)arrowPathInRect:(CGRect)rect direction:(LYPopupMenuArrowDirection)direction
{
    // Create arrow path
...
- (void)drawArrowInRect:(CGRect)rect direction:(LYPopupMenuArrowDirection)direction highlighted:(BOOL)highlighted
{
    ...
    // Separator
    if ((direction == LYPopupMenuArrowDirectionDown || direction == LYPopupMenuArrowDirectionUp) & self.showSeparator) {
        for (LYPopupMenuItemView *itemView in self.visibleItemViews) {
...
- (void)drawBodyInRect:(CGRect)rect firstItem:(BOOL)firstItem lastItem:(BOOL)lastItem highlighted:(BOOL)highlighted
{
    ...
    // Separator
    if (!lastItem && self.showSeparator) {
